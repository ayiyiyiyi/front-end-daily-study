...
> 起因是看到一道关于 js 中变量提升的面试题，在做题是做错了，并且不知道为什么，发现自己对于 js 执行环境、作用域 并不是十分的清楚，以及 js 的执行过程也并不是很清楚，所以不知道为什么 var 会使得变量提升。虽然之前看了关于执行过程的文章，但果然不系统的看一遍和整理一遍是不理解的。所以整理一下，以后也好再次温习。
